"""modules/derms/ui.py — DERMS Explorer: voltage, DER impact, hosting capacity."""
...
from feeder_io import ANSI_LO, ANSI_HI


# ── Colour helpers ─────────────────────────────────────────────────────────────

def _v_color(v: float) -> str:
    if v < ANSI_LO:   return "#B71C1C"
    if v > ANSI_HI:   return "#E65100"
    if v < 0.97:      return "#F57F17"
    if v > 1.03:      return "#F9A825"
    return "#2E7D32"
...
of PV to manage peak load, congestion, or frequency events.
    ...
    with tab_net:
        ...
        fig.update_layout(
            uirevision=f"derms_net_{feeder_name}",
        ...
        with st.expander("Capacitor banks and load summary"):
            ...
            with cc1:
                ...
                st.dataframe(pd.DataFrame(caps), hide_index=True, use_container_width=True)
            with cc2:
                ...
                st.dataframe(ph_sum, hide_index=True, use_container_width=True)
    ...
    with tab_vp:
        ...
        if _large_vp:
            ...
            for ph in [1, 2, 3]:
                ...
                vfig.add_trace(go.Scatter(
                    x=bus_labels, y=voltages, mode="markers",
                    name=ph_names[ph],
        ...
        with st.expander("All buses — sorted by voltage"):
            ...
            st.dataframe(pd.DataFrame(flat_rows), hide_index=True, use_container_width=True)
    ...
    with tab_der:
        ...
        if dr is not None:
            ...
            cfig.update_layout(
                barmode="overlay",
                xaxis_title="Bus (sorted by ΔV, largest first)",
            ...
            nfig.update_layout(
                ...
                title=dict(
                    text="Green = voltage rose  |  Red = voltage dropped  |  ★ = PV bus",
                    font=dict(size=11), x=0.5, xanchor="center",
                ),
            )
...
violation appears.
        ...
        if sweep_btn:
            ...
            _proc = subprocess.Popen(
                _hc_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True,
            )
            ...
            if _proc.returncode != 0:
                ...
                st.session_state[hc_key] = load_pkl(_pkl)
                import os; os.unlink(_pkl)
